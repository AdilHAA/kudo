...
def forecast_sales_autogluon(time_series_df, periods=12, target='final_price', item_id_col='item_id', timestamp_col='date_key'):
    """
    ...
    try:
        ...
        predictor = TimeSeriesPredictor(
            ...
            path=f"autogluon_model_{target}",
            target=target,
            eval_metric="MAPE",
...
def forecast_sales(time_series, periods=12, method='prophet', target='final_price', **kwargs):
    ...
    if method == 'prophet':
        ...
        return forecast_sales_autogluon(time_series, periods, target, item_id_col, timestamp_col)
    elif method == 'average':
